...
const ensureThread = async () => {
  ...
  const payload = await jsonRequest("/api/thread/start", "POST", {
...
const sendPrompt = async () => {
  ...
  await jsonRequest("/api/turn/start", "POST", {
    threadId,
    text,
    app
  });
...
const mountWidgetFrame = async (container, item, templateUri) => {
  const response = await jsonRequest("/api/mcpServer/resource/read", "POST", {
    server: item.server,
    uri: templateUri
  });
...
const renderMcpCall = async (item) => {
  ...
  if (uiAllowed && templateUri) {
    await mountWidgetFrame(card, item, templateUri);
  } else {
...
window.addEventListener("message", async (event) => {
  ...
  if (message.method === "tools/call") {
    ...
    try {
      await jsonRequest("/api/bridge/tools/call", "POST", {
        threadId: state.threadId,
        toolName: name,
        arguments: args,
